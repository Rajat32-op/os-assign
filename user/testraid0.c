...
void test_bounds() {
    ...
    int pid_A = fork();
    if (pid_A == 0) {
        // CPU bound work to drop priority
        volatile long x = 0;
    ...
    int pid_B = fork();
    if (pid_B == 0) {
        // I/O bound work to keep priority high
    ...
    // Parent forces eviction while A and B are sleeping
    int parent_pages = 60; 
    char *pb = sbrklazy(parent_pages * PGSIZE);
    for (int i = 0; i < parent_pages; i++) pb[i * PGSIZE] = 'P';
    
    pause(10); // Let evictions settle
...
int main(void) {
    printf("=== Starting RAID 0 Test ===\n");
    if(setraid(0) < 0) {
        printf("  [FAIL] Failed to set RAID 0\n");
        exit(1);
    }
    ...
    if (errs == 0) {
        printf("  [PASS] Data intact after eviction\n");
    } else {
