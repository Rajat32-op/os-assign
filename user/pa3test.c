...
int main(void) {
    ...
    if (pid_cpu == 0) {
        ...
        char *mb = sbrklazy(NFRAMES * PGSIZE);
        for (int i = 0; i < NFRAMES; i++) mb[i * PGSIZE] = 1;
        pause(50); exit(0);
    }
    int pid_sys = fork();
    if (pid_sys == 0) {
        ...
        char *mb = sbrklazy(NFRAMES * PGSIZE);
        for (int i = 0; i < NFRAMES; i++) mb[i * PGSIZE] = 2;
        pause(50); exit(0);
    }
    pause(10);
    char *pb = sbrklazy((NFRAMES / 2) * PGSIZE);
    for (int i = 0; i < NFRAMES / 2; i++) pb[i * PGSIZE] = 3;
    pause(5);
